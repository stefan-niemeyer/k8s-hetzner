...
settings_worker_selector=${settings_worker_selector:-usage=k8s-vm}
SERVER_LIST=$(hcloud server list -l "group=${settings_group}" -l "${settings_worker_selector}" -o columns=name,ipv4 -o noheader)
while read -r SERVER_INFOS; do
  ...
  (
    scp -i "${settings_vm_id_file}" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "${PROJECT_DIR}"/worker-scripts/* "root@${PUBLIC_IP}:" \
    ...
    && for VM_USER in "${VM_USERS[@]}"; do
         ...
         "${SCRIPT_DIR}/create_guac_user.sh" "${settings_guac_admin}" "${settings_guac_passwd}" "${VM_USER}" "${settings_lab_passwd}"
       done
  ) &
done <<< "${SERVER_LIST}"
