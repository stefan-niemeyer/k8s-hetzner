...
curl -ks -X PATCH "${GUAC_URL}/api/session/data/postgresql/users/${NEW_ADMIN_USER}/permissions?token=${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '[
    {
      "op": "add",
...
curl -ks -X DELETE "${GUAC_URL}/api/session/data/postgresql/users/${ADMIN_USER}?token=${NEW_TOKEN}" > /dev/null
